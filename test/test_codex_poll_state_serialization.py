...
def test_codex_poll_state_imports_v1_without_schema_version() -> None:
    ...
    assert poll.bound_task_id == "task-1"
    assert poll.requires_task_id is False
    assert poll.task_id_probe_cache_key is None
    ...
    poll = from_runtime_state(
        {
            **_v1_runtime_state(),
            ...
            "task_id_probe_cache_key": "codex:/usr/bin/codex:mtime",
        }
    )
...
def test_codex_poll_state_missing_requires_task_id_defaults_false() -> None:
    ...
    poll = from_runtime_state(
        {
            **_v1_runtime_state(),
            ...
            "requires_task_id": True,
            "task_id_probe_cache_key": "codex:/usr/bin/codex:version:mtime",
        }
    )

    assert poll.schema_version == CODEX_POLL_STATE_SCHEMA_VERSION + 9
    assert poll.requires_task_id is True
    assert poll.bound_task_id == "task-1"
...
def test_codex_poll_state_forward_version_fails_closed_without_identity_fields() -> None:
    try:
        ...
        assert "requires task identity" in str(exc)
    else:
        raise AssertionError("forward schema with requires_task_id must fail without bound_task_id")


def test_codex_poll_state_chained_migration_v1_to_v3_fixture() -> None:
    ...
    def migrate_v2_to_v3(payload: dict[str, object]) -> dict[str, object]:
        ...
        return {**payload, "schema_version": 3, "future_only_field": "dropped"}

    with temporary_poll_state_migration(2, 3, migrate_v2_to_v3):
        poll = from_runtime_state(_v1_runtime_state(), target_schema_version=3)

    assert migration_trace == [2]
    assert poll.schema_version == 3
    assert to_runtime_state(poll)["schema_version"] == 3
    assert "future_only_field" not in to_runtime_state(poll)


def test_build_poll_state_delegates_to_schema_migration() -> None:
    poll = build_poll_state(_submission(_v1_runtime_state()))
...
def test_finalization_persists_state_through_serializer() -> None:
    poll = CodexPollState(
        ...
        bound_task_id="task-1",
        requires_task_id=True,
        task_id_probe_cache_key="codex:/usr/bin/codex:v1:123",
    )
    ...
    assert runtime_state["task_id_probe_cache_key"] == "codex:/usr/bin/codex:v1:123"
